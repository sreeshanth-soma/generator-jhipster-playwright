...
jhipster jdl app.jdl --blueprints playwright --skip-install --force --skip-git --no-insight
...
PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 npm install --save-dev "${blueprint_tarball}"
...
ready=0
for _ in {1..90}; do
  if curl -fsS http://127.0.0.1:8080/management/health >/dev/null; then
    ready=1
    break
  fi
  sleep 2
